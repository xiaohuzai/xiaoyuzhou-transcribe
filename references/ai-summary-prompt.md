...
**⚠️ 嘉宾主导型播客修正规则**（商业访谈、纪录片对谈）：
- Step 2 输出可能把 90% 段落都标成"主持人"——这是 inherit 机制导致，**不是真实说话人分布**
- 修正时优先按整段首词 + 段尾标点判断（见 `references/speaker-detection-strategies.md` 末尾章节），不要盲目信任关键词表
- 这种播客的真相：**主持人只问 5-8 个问题，嘉宾讲 30-50 分钟**——LLM 必须理解这个分布后再分配
